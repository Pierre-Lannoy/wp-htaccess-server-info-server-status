...
if ( ! class_exists( 'PerfOpsOne\AdminMenus' ) ) {
	class AdminMenus {
		...
		public static function initialize() {
			foreach ( apply_filters( 'init_perfops_admin_menus', [] ) as $menu => $submenus ) {
				if ( ! in_array( 'perfopsone-' . $menu, self::$slugs, true ) ) {
					switch ( $menu ) {
						case 'analytics':
							add_menu_page( esc_html__( 'Dashboard', 'htaccess-server-info-server-status' ), sprintf( esc_html__( '%s Analytics', 'htaccess-server-info-server-status' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 'dashicons-chart-bar', 210874 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Dashboard', 'htaccess-server-info-server-status' ), __( 'Dashboard', 'htaccess-server-info-server-status' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 0 );
							break;
						case 'tools':
							add_menu_page( esc_html__( 'Available Tools', 'htaccess-server-info-server-status' ), sprintf( esc_html__( '%s Tools', 'htaccess-server-info-server-status' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 'dashicons-admin-tools', 210876 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Tools', 'htaccess-server-info-server-status' ), __( 'Available Tools', 'htaccess-server-info-server-status' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 0 );
							break;
						case 'insights':
							add_menu_page( esc_html__( 'Available Reports', 'htaccess-server-info-server-status' ), sprintf( esc_html__( '%s Insights', 'htaccess-server-info-server-status' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 'dashicons-lightbulb', 210875 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Reports', 'htaccess-server-info-server-status' ), __( 'Available Reports', 'htaccess-server-info-server-status' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 0 );
							break;
						case 'records':
							add_menu_page( esc_html__( 'Available Catalogues', 'htaccess-server-info-server-status' ), sprintf( esc_html__( '%s Records', 'htaccess-server-info-server-status' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 'dashicons-book', 210873 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Catalogues', 'htaccess-server-info-server-status' ), __( 'Available Catalogues', 'htaccess-server-info-server-status' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 0 );
							break;
						case 'settings':
							add_menu_page( esc_html__( 'Control Center', 'htaccess-server-info-server-status' ), sprintf( esc_html__( '%s Settings', 'htaccess-server-info-server-status' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 'dashicons-admin-settings', 210871 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Control Center', 'htaccess-server-info-server-status' ), __( 'Control Center', 'htaccess-server-info-server-status' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 0 );
							break;
						case 'consoles':
							add_menu_page( esc_html__( 'Available Consoles', 'htaccess-server-info-server-status' ), sprintf( esc_html__( '%s Consoles', 'htaccess-server-info-server-status' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 'dashicons-embed-generic', 210872 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Consoles', 'htaccess-server-info-server-status' ), __( 'Available Consoles', 'htaccess-server-info-server-status' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 0 );
							break;
					}
			...
			if ( array_key_exists( 'analytics', self::$menus ) ) {
				...
				foreach ( self::$menus['analytics'] as $item ) {
					...
					if ( $item['activated'] ) {
						...
						$i['text'] = esc_html__( 'This analytics feature is currently disabled. Click here to activate it.', 'htaccess-server-info-server-status' );
						$i['url']  = $item['remedy'];
					}
					$items[] = $i;
				}
		...
		public static function get_settings_page() {
			if ( array_key_exists( 'settings', self::$menus ) ) {
				...
				foreach ( self::$menus['settings'] as $item ) {
					...
					if ( $d->waiting_update() ) {
						$i['need_update'] = sprintf( esc_html__( 'Need to be updated to %s.', 'htaccess-server-info-server-status' ), $d->waiting_update() );
					} else {
						$i['need_update'] = '';
					}
					if ( $d->is_required_wp_ok() ) {
						$i['need_wp_update'] = '';
						$i['ok_wp_update']   = esc_html__( 'OK', 'htaccess-server-info-server-status' );
					} else {
						$i['need_wp_update'] = esc_html__( 'need update', 'htaccess-server-info-server-status' );
						$i['ok_wp_update']   = '';
					}
					if ( $d->is_required_php_ok() ) {
						$i['need_php_update'] = '';
						$i['ok_php_update']   = esc_html__( 'OK', 'htaccess-server-info-server-status' );
					} else {
						$i['need_php_update'] = esc_html__( 'need update', 'htaccess-server-info-server-status' );
						$i['ok_php_update']   = '';
					}
					$i['icon'] = call_user_func( $item['icon_callback'] );
					$i['slug'] = $item['plugin'];
					...
					if ( 0 < (int) $i['installs'] ) {
						$i['installs'] = sprintf( esc_html__( '%s+ installs.', 'htaccess-server-info-server-status' ), Conversion::number_shorten( (int) $i['installs'], 0 ) );
					} else {
						$i['installs'] = '';
					}
					if ( 0 < (int) $i['downloads'] ) {
						$i['downloads'] = sprintf( esc_html__( '%s downloads.', 'htaccess-server-info-server-status' ), Conversion::number_shorten( (int) $i['downloads'], 2 ) );
					} else {
						$i['downloads'] = '';
					}
					if ( 0 < (int) $i['reviews'] ) {
						$i['reviews'] = sprintf( esc_html__( '%s reviews.', 'htaccess-server-info-server-status' ), Conversion::number_shorten( (int) $i['reviews'], 0 ) );
						$i['rating']  = $i['rating'] / 20;
					} else {
						$i['reviews'] = esc_html__( 'No review yet.', 'htaccess-server-info-server-status' );
						$i['rating']  = 0.0;
					}
					$i['stars'] = '';
					if ( 0 < (int) $i['rating'] ) {
		...
		private static function display_as_lines( $items ) {
			...
			foreach ( $items as $item ) {
				$links   = [];
				$links[] = '<a href="' . $item['url'] . '"/>' . __( 'settings', 'htaccess-server-info-server-status' ) . '</a>';
				$links[] = '<a href="' . $item['url'] . '&tab=about"/>' . __( 'about', 'htaccess-server-info-server-status' ) . '</a>';
				$links[] = '<a href="https://wordpress.org/support/plugin/' . $item['slug'] . '" target="_blank">' . __( 'support', 'htaccess-server-info-server-status' ) . '</a>';
				$links[] = '<a href="https://github.com/Pierre-Lannoy/wp-' . $item['slug'] . '" target="_blank">' . __( 'contribution', 'htaccess-server-info-server-status' ) . '</a>';
				$disp   .= '<div class="po-container">';
				$disp   .= ' <div class="po-actionable">';
				...
				$disp   .= '     <span class="po-description">' . $item['text'] . '</span>';
				$disp   .= '     <span class="po-requires">' . sprintf( esc_html__( 'Requires at least PHP %1$s%2$s and WordPress %3$s%4$s.', 'htaccess-server-info-server-status' ), $item['php_version'], '<span class="po-needupdate">' . $item['need_php_update'] . '</span><span class="po-okupdate">' . $item['ok_php_update'] . '</span>', $item['wp_version'], '<span class="po-needupdate">' . $item['need_wp_update'] . '</span><span class="po-okupdate">' . $item['ok_wp_update'] . '</span>' ) . '</span>';
				$disp   .= '    </div>';
