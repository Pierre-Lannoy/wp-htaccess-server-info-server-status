...
class Capture {
	 ...
	 *
	 * @since  1.0.0
	 * @var    \Hsiss\System\Timezone    $local_timezone    The local timezone.
	 */
	...
	public static function init() {
		...
		if ( Option::network_get( 'outbound_capture' ) ) {
			//add_filter( 'pre_http_request', [ 'Hsiss\Plugin\Feature\Capture', 'pre_http_request' ], 10, 3 );
			//add_filter( 'http_api_debug', [ 'Hsiss\Plugin\Feature\Capture', 'http_api_debug' ], 10, 5 );
			$started = true;
		}
		if ( Option::network_get( 'inbound_capture' ) ) {
			//add_filter( 'rest_pre_echo_response', [ 'Hsiss\Plugin\Feature\Capture', 'rest_pre_echo_response' ], 10, 3 );
			$started = true;
		}
		if ( $started ) {
	...
	private static function clean_endpoint( $host, $endpoint, $bound, $cut = 3 ) {
		 ...
		 * @param   string $bound      Maybe 'inbound', 'outbound' or 'unknown'.
		 */
		$cut = (int) apply_filters( 'hsiss_path_level', $cut, $host, $endpoint, $bound );
	...
	private static function inbound_stop( $url, $args ) {
		$stop = microtime( true );
		if ( defined( 'HSISS_INBOUND_CHRONO' ) ) {
			$start = HSISS_INBOUND_CHRONO;
		} else {
			$start = self::$default_chrono;
		}
