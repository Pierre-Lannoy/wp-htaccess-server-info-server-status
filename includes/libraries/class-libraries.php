...
class Libraries {
	...
	public static function init() {
		...
		self::$psr4_libraries['feather']   = [
			'name'    => 'Feather',
			'prefix'  => 'Feather',
			'base'    => HSISS_VENDOR_DIR . 'feather/',
			'version' => '4.24.1',
			// phpcs:ignore
			'author'  => sprintf( esc_html__( '%s & contributors', 'htaccess-server-info-server-status' ), 'Cole Bemis' ),
		...
		self::$psr4_libraries['markdown'] = [
			...
			'base'    => HSISS_VENDOR_DIR . 'markdown/',
			'version' => '1.2.1',
			// phpcs:ignore
			'author'  => sprintf( esc_html__( '%s & contributors', 'htaccess-server-info-server-status' ), 'Carsten Brandt' ),
	...
	private function license_name( $license ) {
		switch ( $license ) {
			case 'mit':
				$result = esc_html__( 'MIT license', 'htaccess-server-info-server-status' );
				break;
			case 'apl2':
				$result = esc_html__( 'Apache license, version 2.0', 'htaccess-server-info-server-status' );
				break;
			case 'gpl2':
				$result = esc_html__( 'GPL-2.0 license', 'htaccess-server-info-server-status' );
				break;
			case 'gpl3':
				$result = esc_html__( 'GPL-3.0 license', 'htaccess-server-info-server-status' );
				break;
			default:
				$result = esc_html__( 'unknown license', 'htaccess-server-info-server-status' );
				break;
		}
		return $result;
	}
	...
	public function sc_get_list( $attributes ) {
		...
		$item['version'] = '3.0.5';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'htaccess-server-info-server-status' ), 'Dan Grossman' );
		...
		$item['name']    = 'Moment';
		$item['version'] = '2.24.0';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'htaccess-server-info-server-status' ), 'Tim Wood' );
		...
		$item['version'] = '1.0.2';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'htaccess-server-info-server-status' ), 'Sam Herbert' );
		...
		$item['version'] = '0.11.4';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'htaccess-server-info-server-status' ), 'Gion Kunz' );
		...
		if ( 'html' === $style ) {
			...
			foreach ( $list as $library ) {
				...
				$items[] = sprintf( __( '<a href="%1$s">%2$s %3$s</a>%4$s by %5$s (%6$s)', 'htaccess-server-info-server-status' ), $library['url'], $library['name'], $library['version'], $library['langs'], $library['author'], $library['license'] );
			}
