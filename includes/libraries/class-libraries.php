...
class Libraries {
	...
	public static function init() {
		...
		self::$psr4_libraries['psr-03']      = [
			'name'    => 'PSR-3',
			'prefix'  => 'Psr\\Log',
			'base'    => HSISS_VENDOR_DIR . 'psr/log-v' . $psrlog_version . '/',
		...
		self::$psr4_libraries['psr-07']      = [
			...
			'base'    => HSISS_VENDOR_DIR . 'psr/http-message/',
			'version' => '2.0',
			'author'  => 'PHP Framework Interop Group',
			'url'     => 'https://www.php-fig.org/',
			'license' => 'mit',
		...
		self::$psr4_libraries['psr-18']      = [
			...
			'base'    => HSISS_VENDOR_DIR . 'psr/http-client/',
			'version' => '1.0.3',
			'author'  => 'PHP Framework Interop Group',
			'url'     => 'https://www.php-fig.org/',
			'license' => 'mit',
		...
		self::$psr4_libraries['decalog-sdk'] = [
			'name'    => 'DecaLog SDK',
			'prefix'  => 'DecaLog',
			'base'    => HSISS_VENDOR_DIR . 'decalog-sdk/',
			'version' => '4.2.0',
	...
	public function sc_get_list( $attributes ) {
		...
		if ( 'html' === $style ) {
			...
			foreach ( $list as $library ) {
				...
				$items[] = sprintf( __( '<a href="%1$s">%2$s %3$s</a>%4$s by %5$s (%6$s)', 'htaccess-server-info-server-status' ), $library['url'], $library['name'], 'v' . $library['version'], $library['langs'], $library['author'], $library['license'] );
			}
