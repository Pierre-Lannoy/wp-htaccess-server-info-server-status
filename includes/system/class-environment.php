...
class Environment {
	...
	public static function init() {
		...
		define( 'HSISS_PLUGIN_URL', $plugin_url );
		define( 'HSISS_PLUGIN_RELATIVE_URL', $plugin_relative_url );
		define( 'HSISS_ADMIN_DIR', HSISS_PLUGIN_DIR . 'admin/' );
		define( 'HSISS_ADMIN_URL', HSISS_PLUGIN_URL . 'admin/' );
		define( 'HSISS_PUBLIC_DIR', HSISS_PLUGIN_DIR . 'public/' );
		define( 'HSISS_PUBLIC_URL', HSISS_PLUGIN_URL . 'public/' );
		define( 'HSISS_INCLUDES_DIR', HSISS_PLUGIN_DIR . 'includes/' );
		define( 'HSISS_VENDOR_DIR', HSISS_PLUGIN_DIR . 'includes/libraries/' );
		define( 'HSISS_LANGUAGES_DIR', HSISS_PLUGIN_DIR . 'languages/' );
		define( 'HSISS_ADMIN_RELATIVE_URL', self::admin_relative_url() );
		define( 'HSISS_AJAX_RELATIVE_URL', self::ajax_relative_url() );
	 ...
	 * @since  1.0.0
	 */
	public static function major_version( $version = HSISS_VERSION ) {
		try {
	 ...
	 * @since  1.0.0
	 */
	public static function minor_version( $version = HSISS_VERSION ) {
		try {
	 ...
	 * @since  1.0.0
	 */
	public static function patch_version( $version = HSISS_VERSION ) {
		try {
			$result = substr( $version, strpos( $version, '.' ) + 1, 1000 );
		...
		if ( defined( 'WP_DEBUG' ) ) {
			if ( WP_DEBUG ) {
				...
				if ( defined( 'WP_DEBUG_LOG' ) ) {
					if ( WP_DEBUG_LOG ) {
						$opt[] = esc_html__( 'log', 'htaccess-server-info-server-status' );
					}
				}
				if ( defined( 'WP_DEBUG_DISPLAY' ) ) {
					if ( WP_DEBUG_DISPLAY ) {
						$opt[] = esc_html__( 'display', 'htaccess-server-info-server-status' );
					}
				}
				$s = implode( ', ', $opt );
			}
		}
		return ( $debug ? esc_html__( 'Debug enabled', 'htaccess-server-info-server-status' ) . ( '' !== $s ? ' (' . $s . ')' : '' ) : esc_html__( 'Debug disabled', 'htaccess-server-info-server-status' ) );
	}
	...
	public static function plugin_version_id() {
		return HSISS_PRODUCT_SHORTNAME . '/' . HSISS_VERSION;
	}
	...
	public static function plugin_version_text() {
		$s = HSISS_PRODUCT_NAME . ' ' . HSISS_VERSION;
		if ( defined( 'HSISS_CODENAME' ) ) {
			if ( HSISS_CODENAME !== '"-"' ) {
				$s .= ' ' . HSISS_CODENAME;
			}
		}
	...
	public static function is_plugin_in_dev_mode() {
		return ( strpos( HSISS_VERSION, 'dev' ) > 0 );
	}
	...
	public static function is_plugin_in_rc_mode() {
		return ( strpos( HSISS_VERSION, 'rc' ) > 0 );
	}
