...
class Cache {
	 ...
	 */
	private static $pool_name = HSISS_SLUG;
	...
	public static function init() {
		...
		add_action( 'shutdown', [ 'Hsiss\System\Cache', 'log_debug' ], 10, 0 );
		add_filter( 'perfopsone_icache_introspection', [ 'Hsiss\System\Cache', 'introspection' ] );
	}
	...
	public static function introspection( $endpoints ) {
		$endpoints[ HSISS_SLUG ] = [ 'name' => HSISS_PRODUCT_NAME, 'version' => HSISS_VERSION, 'endpoint' => [ 'Hsiss\System\Cache', 'get_analytics' ] ];
		return $endpoints;
	}
		...
		if ( self::$apcu_available ) {
			if ( function_exists( 'apcu_cache_info' ) && function_exists( 'apcu_delete' ) ) {
				try {
					...
					\DecaLog\Engine::eventsLogger( HSISS_SLUG )->error( sprintf( 'Unable to query APCu status: %s.', $e->getMessage() ), [ 'code' => $e->getCode() ] );
					$result = 0;
				}
			}
		} elseif ( wp_using_ext_object_cache() ) {
	...
	public static function log_debug() {
		...
		if ( 0 !== (int) $analytics['hit']['count'] || 0 !== (int) $analytics['miss']['count'] ) {
			\DecaLog\Engine::eventsLogger( HSISS_SLUG )->debug( $log );
		}
	}
