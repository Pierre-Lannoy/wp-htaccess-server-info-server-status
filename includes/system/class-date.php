...
class Date {
	...
	public static function get_positive_time_diff_from_mysql_utc( $from ) {
		if ( strtotime( $from ) < time() ) {
			return sprintf( esc_html__( '%s ago', 'htaccess-server-info-server-status' ), human_time_diff( strtotime( $from ) ) );
		} else {
			return esc_html__( 'currently', 'htaccess-server-info-server-status' );
		}
	}
	...
	public static function get_time_diff_from_mysql_utc( $from ) {
		if ( strtotime( $from ) < time() ) {
			...
			return sprintf( esc_html__( 'in %s', 'htaccess-server-info-server-status' ), human_time_diff( strtotime( $from ) ) );
		}
	}
	...
	public static function get_age_array_from_seconds( $age, $legend = false, $abbrev = false ) {
		if ( $abbrev ) {
			$intervals = [
				[ 60, _x( 'sec', 'Unit abbreviation - Stands for "second".', 'htaccess-server-info-server-status' ), _x( 'sec', 'Unit abbreviation - Stands for "second".', 'htaccess-server-info-server-status' ) ],
				[ 60, _x( 'min', 'Unit abbreviation - Stands for "minute".', 'htaccess-server-info-server-status' ), _x( 'min', 'Unit abbreviation - Stands for "minute".', 'htaccess-server-info-server-status' ) ],
				[ 100000, _x( 'hr', 'Unit abbreviation - Stands for "hour".', 'htaccess-server-info-server-status' ), _x( 'hr', 'Unit abbreviation - Stands for "hour".', 'htaccess-server-info-server-status' ) ],
			];
		} else {
			$intervals = [
				[ 60, _n( 'second', 'seconds', 1, 'htaccess-server-info-server-status' ), _n( 'second', 'seconds', 2, 'htaccess-server-info-server-status' ) ],
				[ 60, _n( 'minute', 'minutes', 1, 'htaccess-server-info-server-status' ), _n( 'minute', 'minutes', 2, 'htaccess-server-info-server-status' ) ],
				[ 100000, _n( 'hour', 'hours', 1, 'htaccess-server-info-server-status' ), _n( 'hour', 'hours', 2, 'htaccess-server-info-server-status' ) ],
			];
		}
		$value = [];
