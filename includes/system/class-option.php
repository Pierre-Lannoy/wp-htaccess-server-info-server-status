...
class Option {
	...
	public static function site_get( $option, $default = null ) {
		...
		return get_option( HSISS_PRODUCT_ABBREVIATION . '_' . $option, $default );
	}
	...
	public static function network_get( $option, $default = null ) {
		...
		return get_site_option( HSISS_PRODUCT_ABBREVIATION . '_' . $option, $default );
	}
	...
	public static function site_exists( $option ) {
		return 'non_existent_option' !== get_option( HSISS_PRODUCT_ABBREVIATION . '_' . $option, 'non_existent_option' );
	}
	...
	public static function network_exists( $option ) {
		return 'non_existent_option' !== get_site_option( HSISS_PRODUCT_ABBREVIATION . '_' . $option, 'non_existent_option' );
	}
	...
	public static function site_set( $option, $value, $autoload = null ) {
		return update_option( HSISS_PRODUCT_ABBREVIATION . '_' . $option, $value, $autoload );
	}
	...
	public static function network_set( $option, $value ) {
		if ( false === $value ) {
			update_site_option( HSISS_PRODUCT_ABBREVIATION . '_' . $option, true );
		}
		return update_site_option( HSISS_PRODUCT_ABBREVIATION . '_' . $option, $value );
	}
	...
	public static function site_delete_all() {
		...
		$result = 0;
		// phpcs:ignore
		$delete = $wpdb->get_col( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE '" . HSISS_PRODUCT_ABBREVIATION . '_%' . "';" );
