...
class Assets {
	...
	public function prefetch() {
		if ( Option::network_get( 'use_cdn' ) && HSISS_CDN_AVAILABLE ) {
	...
	public function register_style( $handle, $src, $file, $deps = [], $media = 'all' ) {
		if ( Option::network_get( 'use_cdn' ) && HSISS_CDN_AVAILABLE ) {
			if ( HSISS_ADMIN_URL === $src ) {
				$file = 'https://cdn.jsdelivr.net/wp/' . HSISS_SLUG . '/tags/' . HSISS_VERSION . '/admin/' . $file;
			} else {
				$file = 'https://cdn.jsdelivr.net/wp/' . HSISS_SLUG . '/tags/' . HSISS_VERSION . '/public/' . $file;
			}
			// phpcs:ignore
