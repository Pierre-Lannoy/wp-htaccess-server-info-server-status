...
class Updater {
	...
	public function __construct() {
		...
		if ( HSISS_VERSION !== $old ) {
			if ( '0.0.0' === $old ) {
				$this->install();
				// phpcs:ignore
				$message = sprintf( esc_html__( '%1$s has been correctly installed.', 'htaccess-server-info-server-status' ), HSISS_PRODUCT_NAME );
			} else {
				$this->update( $old );
				// phpcs:ignore
			...
			Option::network_set( 'version', HSISS_VERSION );
		}
	}
	...
	public function sc_get_changelog( $attributes ) {
		...
		$error       = esc_html__( 'Sorry, unable to find or read changelog file.', 'htaccess-server-info-server-status' );
		$result      = esc_html( $error );
		$changelog   = HSISS_PLUGIN_DIR . 'CHANGELOG.md';
